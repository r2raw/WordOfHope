...
function AddNurseSched(props) {
  ...
  useEffect(() => {
    const isAnyArrayCompleted = employeeSched.some(schedule => {
      return schedule.startTime !== "" && schedule.endTime !== "" && schedule.days.length > 0;
    });
  ...
  const handleDeleteSelection = (index) => {
    ...
    setSelectedDays(prevSelectedDays => prevSelectedDays.filter(day => !deletedDays.includes(day)));
  };
...
const handleCheckChange = (selectedDay, empSchedIndex) => {
  ...
  return (
    <form className="scheduler-form" onSubmit={handleSubmit}>
      <div onClick={handleAddSelection} style={selectedDays.length === 7 || employeeSched.length === 7 ?{display: "none"} :{}} className="add-button card">
        <AddCircleSharpIcon />
      </div>
      ...
      <button className="solid button fade submit-schedule" disabled={disabled}>Submit</button>
    </form>
  );
}
