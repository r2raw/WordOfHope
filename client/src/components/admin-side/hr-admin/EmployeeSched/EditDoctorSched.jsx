    ...
    if (itemsToCheck.length > 0) {
      for (let i = 0; i < itemsToCheck.length; i++) {
        for (let j = 0; j < days.length; j++) {
          if (
            itemsToCheck[i].days.includes(
              weekdays[(weekdays.indexOf(days[j]) - 1 + 7) % 7]
            )
          ) {
            ...
            itemsToCheck[i].days.includes(
              weekdays[(weekdays.indexOf(days[j]) + 1) % 7]
            )
          ) {
            let checkEndtime = dayjs(
      ...
      if (empSchedArray.length > 1) {
        ...
        const updatedEmpSchedArray = empSchedArray.map((empSched, index) => {
          ...
          if (
            ...
            dayjs("1970-01-02T" + startTime).format("YYYY-MM-DD HH:mm:ss")
          ) {
